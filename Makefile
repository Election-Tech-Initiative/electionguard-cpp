...
build-android:
	cmake -S . -B $(ELECTIONGUARD_BUILD_LIBS_DIR)/android/arm64-v8a \
		...
		-DCMAKE_ANDROID_ARCH_ABI=arm64-v8a -DCMAKE_SYSTEM_VERSION=21
	...
	cmake -S . -B $(ELECTIONGUARD_BUILD_LIBS_DIR)/android/armeabi-v7a \
		...
		-DCMAKE_ANDROID_ARCH_ABI=armeabi-v7a -DCMAKE_SYSTEM_VERSION=21 \
	...
	cmake -S . -B $(ELECTIONGUARD_BUILD_LIBS_DIR)/android/x86 \
		...
		-DCMAKE_SYSTEM_NAME=Android \
		-DCMAKE_ANDROID_NDK=$(NDK_PATH) \
		-DCMAKE_ANDROID_ARCH_ABI=x86 -DCMAKE_SYSTEM_VERSION=21
	cmake --build $(ELECTIONGUARD_BUILD_LIBS_DIR)/android/x86
	cmake -S . -B $(ELECTIONGUARD_BUILD_LIBS_DIR)/android/x86_64 \
		-DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON \
		-DCMAKE_SYSTEM_NAME=Android \
		-DCMAKE_ANDROID_NDK=$(NDK_PATH) \
		-DCMAKE_ANDROID_ARCH_ABI=x86_64 -DCMAKE_SYSTEM_VERSION=21
	cmake --build $(ELECTIONGUARD_BUILD_LIBS_DIR)/android/x86_64

build-ios:
	cmake -S . -B $(ELECTIONGUARD_BUILD_LIBS_DIR)/ios -G Xcode \
		-DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=$(IOS_TOOLCHAIN) \
		-DPLATFORM=OS64COMBINED -DDEPLOYMENT_TARGET=12.0
	cmake --build $(ELECTIONGUARD_BUILD_LIBS_DIR)/ios --config Release --target install
