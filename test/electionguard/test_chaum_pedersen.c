...
static bool test_disjunctive_chaum_pedersen_proof(void);
static bool test_disjunctive_chaum_pedersen_proof_deterministic(void);
static bool test_constant_chaum_pedersen_proof(void);

bool test_chaum_pedersen_proof(void)
{
    printf("\n -------- test_chaum_pedersen.c --------- \n");
    return test_disjunctive_chaum_pedersen_proof() &&
           test_disjunctive_chaum_pedersen_proof_deterministic() &&
           test_constant_chaum_pedersen_proof();
}

bool test_disjunctive_chaum_pedersen_proof(void)
{
    printf("\n -------- test_disjunctive_chaum_pedersen_proof -------- \n");
    ...
    if (eg_disjunctive_chaum_pedersen_proof_make(message, nonce, public_key, one_mod_q, 0UL,
                                                 &proof)) {
        assert(false);
    }
    eg_disjunctive_chaum_pedersen_proof_t *bad_proof = NULL;
    if (eg_disjunctive_chaum_pedersen_proof_make(message, nonce, public_key, one_mod_q, 1UL,
                                                 &bad_proof)) {
        assert(false);
    }

    // Assert
    assert(eg_disjunctive_chaum_pedersen_proof_is_valid(proof, message, public_key, one_mod_q) ==
           true);
    assert(eg_disjunctive_chaum_pedersen_proof_is_valid(bad_proof, message, public_key,
                                                        one_mod_q) == false);
    ...
    if (eg_disjunctive_chaum_pedersen_proof_free(proof)) {
        assert(false);
    }
    if (eg_disjunctive_chaum_pedersen_proof_free(bad_proof)) {
        assert(false);
    }
...
bool test_disjunctive_chaum_pedersen_proof_deterministic(void)
{
    printf("\n -------- test_disjunctive_chaum_pedersen_proof_deterministic -------- \n");
    ...
    // Act
    eg_disjunctive_chaum_pedersen_proof_t *proof = NULL;
    if (eg_disjunctive_chaum_pedersen_proof_make_deterministic(message, nonce, public_key,
                                                               one_mod_q, seed, 0UL, &proof)) {
        assert(false);
    }
    eg_disjunctive_chaum_pedersen_proof_t *bad_proof = NULL;
    if (eg_disjunctive_chaum_pedersen_proof_make_deterministic(message, nonce, public_key,
                                                               one_mod_q, seed, 1UL, &bad_proof)) {
        assert(false);
    }
