...
bool test_chaum_pedersen_proof(void)
{
    ...
    return test_disjunctive_chaum_pedersen_proof() && test_constant_chaum_pedersen_proof();
}
...
bool test_disjunctive_chaum_pedersen_proof(void)
{
    ...
    if (eg_disjunctive_chaum_pedersen_proof_make(message, nonce, public_key, one_mod_q, seed, 0UL,
                                                 &proof)) {
        assert(false);
    }
    eg_disjunctive_chaum_pedersen_proof_t *bad_proof = NULL;
    if (eg_disjunctive_chaum_pedersen_proof_make(message, nonce, public_key, one_mod_q, seed, 1UL,
                                                 &bad_proof)) {
        assert(false);
    }
